CREATE TABLE search (
    ...
    previous_link_location VARCHAR(256) NOT NULL,
    next_link_location VARCHAR(256) NOT NULL
);
...
CREATE TABLE value_evaluation_definition (
    ...
    CHECK (
        ...
        AND result_processing_engine IN ('REG_EXP', 'STRING_SEARCH', 'CSS_QUERY_SEARCH'))
);
...
CREATE TABLE value_evaluation_in_result_entry_mapping (
    ...
    UNIQUE KEY (destination, position, result_entry_definition_id, result_entry_definition_id)
);
